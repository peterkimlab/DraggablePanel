 ...
 */
class DraggableViewCallback2 extends ViewDragHelper.Callback {
  ...
  private static boolean firstDraggaleView= true;
   ...
   */
  public DraggableViewCallback2(DraggableView2 draggableView, View draggedView) {
  ...
  private void triggerOnReleaseActionsWhileVerticalDrag(float yVel) {
    if (yVel < 0 && yVel <= -Y_MIN_VELOCITY) {
      ...
      if (draggableView.isDragViewAboveTheMiddle()) {
        if(!draggableView.isDragViewAtTop()){
          draggableView.maximize();
        } else if(firstDraggaleView){
          firstDraggaleView = false;
          draggableView.maximize();
        }
      } else {
        draggableView.minimize();
